...
const generateAnnualRevenue = () => {
    const categories = ["thousand", "million", "billion"];
    const category = categories[Math.floor(Math.random() * categories.length)];

    // Generate random number depending on category
    let value;
    switch (category) {
        case "thousand":
            value = (Math.random() * (999 - 10) + 10).toFixed(1); // 10.0–999.0 thousand
            break;
        case "million":
            value = (Math.random() * (900 - 1) + 1).toFixed(1); // 1.0–900.0 million
            break;
        case "billion":
            value = (Math.random() * (50 - 1) + 1).toFixed(1); // 1.0–50.0 billion
            break;
    }
...
const updateRandomData = async(req, res) => {
    try {
        const docs = await Assignment.find();
        console.log(`${docs.length} total docs found`);
        ...
        const updates = docs.map(doc => ({
            updateOne: {
                ...
                update: {
                    $set: {
                        annualRevenue: generateAnnualRevenue(),
                        cleanCode: generateClientCode(),
                    },
                },
            },
        }));

        const result = await Assignment.bulkWrite(updates);
